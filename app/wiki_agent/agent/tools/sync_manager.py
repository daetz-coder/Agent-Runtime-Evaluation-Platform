...
class WikiSyncManager:
    ...
    def embedding_model(self):
        ...
        if self._embedding_model is None:
            from app.wiki_agent.agent.tools.embeddings import get_embedding_model
    ...
    def reindex_all(self) -> dict:
        ...
        for md_file in knowledge_dir.rglob("*.md"):
            ...
            try:
                page = service.get_page(rel_path)
                self._sync_to_vector_store(rel_path, page.title, page.content, page.tags or [])
                count += 1
            except Exception as exc:
                print(f"[WikiSync] 重建失败 {md_file}: {exc}")
    ...
    def reindex_page(self, path: str) -> dict:
        ...
        try:
            ...
            return {"status": "error", "message": f"索引同步失败: {str(exc)}"}
