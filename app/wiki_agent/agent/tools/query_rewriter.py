"""Query 改写 Pipeline — 复杂度分级 + 上下文补齐 + 合并分类改写 + 相似度校验

Pipeline:
    1. classify_complexity — 规则分级（trivial/simple/medium/complex）
    2. Contextualizer — 多轮对话指代消解（检测到代词才触发）
    3. _classify_and_rewrite — 单次 LLM 同时完成分类与改写
