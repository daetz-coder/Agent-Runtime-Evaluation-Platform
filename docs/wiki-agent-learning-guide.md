...
第八阶段: eval_middleware.py
           评估集成：如何自动采集 Agent 轨迹
```
...
### 8.1 `app/wiki_agent/agent/eval_middleware.py` ⭐

| 关注点 | 说明 |
|--------|------|
| 读什么 | 评估中间件——SDK 与业务代码的桥接层 |
| 重点 | `instrument_graph()`（自动采集节点执行）、`wrap_llm()`（自动采集 LLM 调用）、`start_session()`/`finish_session()`（评估生命周期）、`record_retrieval()`（手动记录检索事件） |
| 关键认知 | 业务代码（graph.py）不直接 import SDK，所有评估相关操作通过此中间件 |

**读完你应该知道**：
- `instrument_graph(graph)` → SDK 包裹 LangGraph，自动采集节点执行/状态变化
- `wrap_llm(llm)` → SDK 包裹 LLM，自动采集 prompt/response/latency
- `record_retrieval()` → 手动记录检索事件（SDK 无法自动采集）
- `start_session()` → 创建评估 task，生成结构化计划
...
| 9 | 评估中间件解耦 | 业务代码不 import SDK | `eval_middleware.py` |
