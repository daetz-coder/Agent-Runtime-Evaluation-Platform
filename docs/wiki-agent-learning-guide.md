...
### 8.1 `app/wiki_agent/hooks.py` + agent-hooks SDK ⭐

| 关注点 | 说明 |
|--------|------|
| 读什么 | 生命周期钩子——通过 agent-hooks SDK 接入评估平台 |
| 重点 | `emit_session_start()`、`emit_retrieval()`、`emit_response()`、`emit_session_end()` |
| 关键认知 | 业务代码通过 emit 函数触发事件，默认空操作，评估平台通过 register() 注入实现 |

**读完你应该知道**：
- `emit.retrieval(query, results, ms)` → 记录检索事件
- `emit.response(session_id, response)` → 记录回复
...
| 9 | 生命周期钩子解耦 | 通过 agent-hooks SDK 接入评估，业务代码零侵入 | `hooks.py` |
