...
├─────────────────────────────────────────────────────────────┤
│                      AI / Agent Layer                       │
│  LangGraph (Wiki 状态机) + LangChain (LLM 抽象)             │
│  评估: evaluate_parallel（asyncio.gather ×6 Judge）         │
│  共识: 多模型交叉验证                                       │
├─────────────────────────────────────────────────────────────┤
│                      RAG / Search Layer                     │
...
│  Redis (缓存/限流/SSE claim)  │  部署: Docker Compose        │
│  非流式评估: FastAPI BackgroundTasks                        │
...
| 用途 | 说明 |
|------|------|
| 部署 | 容器化应用，`docker compose` 启动 Backend / Frontend / Redis 等 |
...
| 项目 | 说明 |
|------|------|
| 角色 | `use_stream=false` 时用 FastAPI `BackgroundTasks` 跑评估 |
| UI 路径 | `use_stream=true` + `POST /evaluations/stream` SSE |
| 文件引用 | `app/api/v1/endpoints/evaluation.py` |
...
1. **可选降级**：Redis 不可用时 cache/限流静默降级；评估仍可在进程内走 BackgroundTasks / SSE
