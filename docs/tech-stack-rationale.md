...
### 2.3 异步评估调度（FastAPI BackgroundTasks）

| 候选 | 选择 | 理由 |
|------|------|------|
| **FastAPI BackgroundTasks** | ✅ | 与 ASGI 进程同生命周期，无额外 broker/worker；覆盖本项目非流式评估量级 |
| Celery + Redis | ❌ | 已移除；运维成本高，原先「Redis 在但 worker 未起」会静默卡住 |
| RQ / Dramatiq | ❌ | 引入第二套队列栈，收益有限 |

**关键收益**：
- UI 默认走 SSE（`use_stream=true`），实时推送六维进度
- 非流式请求在同进程内 `_run_evaluation_background` 执行，接口立即 202
- Wiki 对话结束 `finish(auto_run=False)`，任务保持 pending，由任务管理手动评估
...
```python
# 评估侧现固定并行：asyncio.gather × 6 维 Judge
#（已移除 LangGraph 串行 create_evaluation_graph / EVAL_PARALLEL）
from app.graphs.evaluation_graph import evaluate_parallel

scores = await evaluate_parallel(goal, trajectory_steps, context)
```

长耗时评测（数十次 LLM 调用）靠并行缩短 wall-clock；Wiki Agent 侧仍用 LangGraph checkpoint 做 HITL 断点续传。
...
| **Vue 3** | ✅ | Composition API + Vue Router + Element Plus，开箱即用（无 Pinia） |
