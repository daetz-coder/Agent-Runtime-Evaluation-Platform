...
| 表 | 说明 | 关键字段 |
|----|------|---------|
| `agent_tasks` | Agent 任务 | id, goal, context, status, timestamps |
...
**功能**：`evaluate_parallel` / `evaluate_partial` 编排 6 维评估（非 LangGraph StateGraph）。

- **全量评估**（`evaluate_parallel`）：6 个评估器通过 `asyncio.gather` 并发执行（~15–30s）
- **部分评估**（`evaluate_partial`）：只运行指定维度的评估器（用于增量评估）

非流式评估由 FastAPI `BackgroundTasks`（`_run_evaluation_background`）触发，无 Celery。
...
- `finish(auto_run)` — 结束采集并 flush；`auto_run=False`（Wiki 默认）仅上报轨迹、任务保持 pending；`True` 则标 completed 并触发评估
...
│           ├── evaluation.py        # ★ 评估 API（创建、SSE、共识、增量）
...
| 端点 | 说明 |
|------|------|
| `POST /` | 创建评估（`use_stream=false` → BackgroundTasks；`true` → 仅建记录） |
...
│     └─ record_think() → 思考过程                             │
│  ③ await collector.finish(auto_run=...) → flush；Wiki 用 False 保持 pending │
└─────────────────────┬───────────────────────────────────────┘
                      │
                      ▼
┌─────────────────────────────────────────────────────────────┐
...
│                                                             │
│  ① await start(goal, context) → 创建 task_id               │
│  ② record(action_type, detail) → 追加到内存缓冲             │
│  ③ await finish(auto_run) / _flush() → HTTP flush 到平台    │
│                                                             │
│  传输层:                                                     │
│  - HTTP: POST /api/v1/tasks/{id}/trajectory（批量上传）      │
│  - 异步: async httpx，不阻塞事件循环                         │
│  - 失败回退: 内存缓冲 + 指数退避重试                         │
└─────────────────────┬───────────────────────────────────────┘
                      │
                      ▼
...
│  ② auto_run=True 时触发评估；Wiki 默认 False → 任务 pending │
│  ③ 6 维评估 → 返回评估结果                                   │
└─────────────────────────────────────────────────────────────┘
```
...
| 方法 | 端点 | 说明 |
|------|------|------|
| POST | `/` | 创建评估（BackgroundTasks 或配合 stream） |
| POST | `/stream` | SSE 流式评估 |
| POST | `/consensus` | 多模型共识评估 |
| POST | `/incremental` | 增量评估 |
| GET | `/{id}` | 获取评估详情 |
...
### 2.3 异步评估：FastAPI BackgroundTasks + Redis（缓存）

| 候选 | 选择 | 理由 |
|------|------|------|
| **BackgroundTasks** | ✅ | 非流式评估入队简单，与 FastAPI 同进程；当前体量足够 |
| Celery + Redis | ❌ | 已移除；额外 broker/worker 运维成本高 |
| asyncio 原生 fire-and-forget | ❌ | 缺少与请求生命周期绑定的清晰边界 |
| RQ / Dramatiq | ❌ | 对本项目当前规模过重 |

**关键收益**：
- `use_stream=false` 时 `_run_evaluation_background` 后台跑完 6 维评估
- Redis 仍用于缓存与限流（与任务队列解耦）
- 流式评估走 SSE，不经过后台队列
...
**① 显式状态图，可审计可调试（Wiki Agent）**

```python
# Wiki Agent 的 LangGraph 表达 — 每个节点的状态转换都是显式的
graph = StateGraph(WikiState)
graph.add_node("search", search)
graph.add_node("respond", respond)
graph.add_node("decide", decide)
graph.add_node("execute", execute)  # HITL interrupt
```

对比黑盒 `agent_executor.invoke()` — 内部状态不可见，出错时难以定位。

> 评估侧生产路径是 `evaluate_parallel` / `evaluate_partial`（`asyncio.gather`），**不是**评估用 LangGraph StateGraph。
...
6 个评测器通过 `asyncio.gather` 并行运行（`evaluate_parallel`）：

```python
# evaluation_graph.py — 生产路径
results = await asyncio.gather(
    *(ev.evaluate(goal, steps, context) for ev in evaluators)
)
```

不依赖 LangGraph `Send()` 做评估编排。

**④ 状态持久化与断点续传（Wiki HITL）**

```python
# Wiki Agent：checkpoint 支持 interrupt / resume
checkpointer = AsyncSqliteSaver(conn=conn)
graph = build_wiki_graph(checkpointer)
result = await graph.ainvoke(initial_state, config)
```

长对话与 HITL 需要断点续传；评估本身是无状态的 `asyncio.gather` 批处理。
**⑤ 与 LangChain 生态无缝集成**
...
| **Vue 3** | ✅ | Composition API + 官方路由（Vue Router）+ Element Plus，开箱即用 |
...
│  Docker (可选部署) + Redis (缓存/限流)                        │
│  BackgroundTasks (异步评估)  │  可观测: OpenTelemetry + Prometheus │
├─────────────────────────────────────────────────────────────┤
│                      Quality Assurance                      │
...
| 项目 | 说明 |
|------|------|
| 角色 | 非流式评估经 FastAPI `BackgroundTasks`（`_run_evaluation_background`） |
| 说明 | 已移除 Celery；流式评估走 SSE |
---
...
1. **可选降级**：Redis 挂掉时核心功能不受影响（cache miss 回源）；评估用 BackgroundTasks，无 Celery 依赖
...
流式评估走 SSE。生产评估路径为 `evaluate_parallel` / `evaluate_partial`（`asyncio.gather`）。
...
- `POST /evaluations/`、`POST /evaluations/stream`、`POST /evaluations/consensus`、`POST /evaluations/incremental`
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST /evaluations/` | 创建并运行评估（`use_stream=false` 时 BackgroundTasks） |
...
# 3. 运行评估（非流式走 BackgroundTasks；也可用 /evaluations/stream）
curl -X POST http://localhost:8000/api/v1/evaluations/ \
  -H "Content-Type: application/json" \
  -d '{"task_id": "{task-uuid}"}'

# 返回: {"id": "eval-uuid", "status": "pending"|"in_progress", ...}
```
...
## 13. 异步评估（BackgroundTasks）

非流式评估经 FastAPI `BackgroundTasks` 执行（无 Celery worker）：

```bash
# 单进程即可：uvicorn / python -m app.main
# use_stream=false → _run_evaluation_background
# use_stream=true  → SSE 流式推送
```
