...
- [五、数据流](#五数据流)
  - [5.1 Sandbox 评测模式数据流](#51-sandbox-评测模式数据流)
...
| `SANDBOX_MOCK_MODE` | `true` | Mock 模式（无需 Docker） |
...
| Sandbox | `SANDBOX_SESSIONS_ACTIVE` / `SANDBOX_SESSION_POOL_SIZE` | 沙箱会话池状态 |
...
| `SandboxEvalRequest` / `SandboxEvalResponse` | 沙箱评测请求/响应 |
...
│   ├── runner.py                    # ★ AgentRunner 顶层编排（沙箱→工作区→Agent循环→轨迹→清理）
...
│   ├── tools/                       # Agent 工具集
│   │   ├── base.py                  # SandboxTool 基类 + ToolProxy（统一网关）
...
#### `agent_runtime/graph.py` — Agent ReAct 循环
...
**完整流程**：
1. Mock 模式检查（如果 `SANDBOX_MOCK_MODE=true`，返回预定义轨迹）
...
#### `agent_runtime/trajectory_recorder.py` — 轨迹记录器
...
#### `agent_runtime/tools/base.py` — 工具代理

**功能**：`ToolProxy` 是所有工具调用的统一网关。
