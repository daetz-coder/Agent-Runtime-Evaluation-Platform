...
**核心能力**：
1. **SDK 轨迹评测**：外部/内嵌 Agent 通过 `TrajectoryCollector` 上报轨迹后评估（无 Docker 沙箱执行路径）
...
│  │  /api/v1/benchmark    /api/v1/system           │  │
...
Agent 执行过程中:
  ...
  collector.record_replan(reason="...", new_plan={...})
     │
     ▼
await collector.finish(auto_run=True)   # 显式 auto_run；Wiki 用 False 仅 flush
  ① POST /api/v1/tasks/{id}/trajectory → 批量上传轨迹
  ② auto_run=True：标 completed + POST /evaluations/ 触发评估
  ③ auto_run=False：任务保持 pending，Tasks UI 手动评估
     │
     ▼
  评估结果通过 API 返回
```
