# Wiki Agent — 完整技术文档（合集版 v3.1）

> 最后更新：2026-07-06
> 本文档整合项目所有技术文档，完整保留原始内容，覆盖背景、架构、实现、难点、面试问答。
> 总计 7 个源文档，包含完整的代码片段、架构图、配置说明。
> v3.1 修订：移除不存在的 hooks.py 描述，修正预算数字，补充缺失组件和配置项。
...
app/wiki_agent/
├── __init__.py                    # 包初始化（空）
├── __main__.py                    # 允许 python -m 方式启动
├── config.py                      # 全局配置（路径、模型、参数）
├── bootstrap.py                   # 启动引导（目录创建、种子数据、索引同步）
├── database.py                    # SQLite 数据库初始化（会话表、消息表、用户记忆表）
│
├── agent/                         # Agent 智能体层
│   ├── __init__.py
...
| `ZHIPUAI_CHAT_MODEL` | `glm-4-flash` | 智谱对话模型名称 |
| `KNOWLEDGE_DIR` | `data/wiki_agent/knowledge` | 知识库 Markdown 文件存储目录 |
| `MILVUS_URI` | `data/wiki_agent/milvus.db` | Milvus 向量数据库路径 |
| `MILVUS_COLLECTION` | `wiki_chunks` | Milvus 集合名称 |
| `EMBEDDING_MODEL` | `BAAI/bge-small-zh-v1.5` | Embedding 模型 |
| `EMBEDDING_MODEL_PATH` | `models/bge-small-zh-v1.5` | Embedding 本地模型路径 |
| `EMBEDDING_DIM` | 512 | 向量维度 |
| `DB_PATH` | `data/wiki_agent/chat.db` | SQLite 数据库路径 |
| `BM25_INDEX_PATH` | `data/wiki_agent/bm25_index.pkl` | BM25 索引持久化路径 |
| `HISTORY_MAX_TURNS` | 10 | 对话历史最大轮数 |
| `RERANK_ENABLED` | `true` | 是否启用 Cross-Encoder 重排 |
| `RERANK_MODEL` | `BAAI/bge-reranker-base` | 重排模型名称 |
| `RERANK_MODEL_PATH` | `models/bge-reranker-base` | 重排本地模型路径 |
| `RERANK_CANDIDATE_MULTIPLIER` | 3 | 重排候选倍数 |
| `RERANK_MAX_LENGTH` | 512 | 重排模型最大输入长度 |
| `QUERY_REWRITE_ENABLED` | `true` | 是否启用 Query 改写 |
| `QUERY_REWRITE_SIMILARITY_THRESHOLD` | 0.7 | Query 改写相似度过滤阈值 |
| `QUERY_REWRITE_MAX_QUERIES` | 5 | Query 改写最大查询数 |
| `CACHE_SESSION_TTL` | 300 | 会话缓存过期时间（秒） |
...
5. `preload_embedding_model()` — 预加载 Embedding 模型
6. `preload_reranker_if_enabled()` — 预加载 Cross-Encoder 重排模型
7. `start_env_monitor()` — 启动环境监控器（5 秒轮询检测文件变化）
...
**表结构**：
- `sessions` — 会话表（id, name, key_facts, created_at, updated_at）
- `messages` — 消息表（id, session_id, role, content, wiki_results, extraction, created_at），含外键约束
- `user_memory` — 用户记忆表（id, facts, updated_at），存储跨会话的用户级持久事实
...
| 节点 | 职责 |
|------|------|
| `search` | 统一检索四路记忆（用户记忆 + 会话记忆 + KB + 对话历史），提取 key_facts |
...
**功能**：在 `graph.py` 中直接调用 `sdk.collector` 的方法，记录 Agent 执行轨迹。SDK 不可用时自动降级为空操作。

**采集方式**：`graph.py` 通过 `from sdk.collector import get_collector` 获取收集器实例，在关键节点直接调用：
- `collector.record_retrieval()` — 记录检索事件
- `collector.record_memory_write()` — 记录记忆写入
...
| 阶段 | 组件 | 说明 |
|------|------|------|
| 1 | `classify_complexity()` | 基于规则的复杂度分级（TRIVIAL / SIMPLE / MEDIUM / COMPLEX），不调用 LLM |
| 2 | `_classify_and_rewrite()` | 对 MEDIUM/COMPLEX 查询，合并分类+改写为单次 LLM 调用 |

**复杂度分层策略**（`QueryComplexity` 枚举）：

| 复杂度 | 判断规则 | 处理方式 |
|--------|---------|---------|
| `TRIVIAL` | 简单问候/闲聊（正则匹配） | 跳过 RAG，直接回复 |
| `SIMPLE` | 简单查询（正则匹配） | 单次搜索，不改写，不重排 |
| `MEDIUM` | 中等复杂度 | 1-2 次改写，不重排 |
| `COMPLEX` | 复杂查询 | 完整 Pipeline + 重排 |

**四种改写策略**（`QueryType` 枚举，由 LLM 分类）：
...
**相似度校验**（`SimilarityValidator`）：对改写后的 query 计算与原始 query 的余弦相似度，低于阈值（默认 0.7）的改写被过滤。
...
| `merge_user_memory()` | 合并新用户记忆（去重，最多 30 条） |
...
wiki-agent 在 `graph.py` 中直接调用 `sdk.collector` 方法，自动采集评估数据：
...
### 7.5 为什么在 graph.py 中直接集成评估采集？

- **简洁**：无需额外的中间层，直接在关键节点调用 collector 方法
- **自动采集**：SDK 自动采集 LLM 调用、节点执行、工具调用
- **手动补充**：SDK 无法自动采集的（如检索细节、记忆写入）在 graph.py 中手动记录
- **统一管理**：评估会话的生命周期（start → record → finish）在 graph.py 中统一管理
- **优雅降级**：SDK 不可用时 collector 为空操作，不影响业务逻辑
...
- [难点 4：四路上下文的预算裁剪](#难点-4四路上下文的预算裁剪)
...
LLM 的上下文窗口是有限的。系统需要将四路记忆（用户记忆、会话记忆、RAG 检索结果、对话历史）组装成一个上下文块，但不能超过预算。

### 为什么难

1. **优先级冲突**：四路记忆哪个更重要？如何分配预算？
...
MAX_USER_FACTS_CHARS = 600    # 用户记忆预算
MAX_SESSION_FACTS_CHARS = 400 # 会话记忆预算
MAX_WIKI_CHARS = 1200         # RAG 检索结果预算
MAX_HISTORY_CHARS = 800       # 对话历史预算
# 总预算 ≈ 3000 字符

def build_context_block(ctx):
    blocks = []
    budget = MAX_USER_FACTS_CHARS + MAX_SESSION_FACTS_CHARS + MAX_WIKI_CHARS + MAX_HISTORY_CHARS  # ~3000

    # 优先级 1: 用户记忆（固定保留，最重要）
    if ctx.user_facts:
        user_text = "\n".join(f"{i}. {f}" for i, f in enumerate(ctx.user_facts, 1))
    ...
    if ctx.session_facts:
        session_text = "\n".join(f"{i}. {f}" for i, f in enumerate(ctx.session_facts, 1))
    ...
    # 优先级 4: history_summary（对话历史，优先级最低）
...
┌─────────────────────────────────────────────┐
│ 优先级 1: 用户记忆（600 字符，固定保留）      │  ← 最重要，跨 session 持久生效
├─────────────────────────────────────────────┤
│ 优先级 2: 会话记忆（400 字符，固定保留）      │  ← 当前 session 生效
├─────────────────────────────────────────────┤
│ 优先级 3: wiki_results（1200 字符）          │  ← 动态：实际用多少扣多少
├─────────────────────────────────────────────┤
│ 优先级 4: history_summary（800 字符）        │  ← 动态：用剩余预算
└─────────────────────────────────────────────┘

如果用户记忆用了 400 字符 → 会话记忆 400，wiki 最多 1200，history 最多 800
如果 wiki_results 用了 1000 字符 → history 最多 800
```
...
> 先理解四路记忆分别是什么（`context_retriever.py` 顶部注释），再看 `build_context_block()` 的预算分配逻辑，最后看 `merge_session_key_facts()` 和 `merge_user_memory()` 的累积淘汰机制。
...
1. **分类准确性**：规则分级和 LLM 分类器都可能分错类，导致使用错误的改写策略
2. **改写质量**：改写后的查询可能偏离原始语义
3. **相似度校验**：需要判断改写是否保留了原始语义
4. **多查询合并**：改写后产生多个查询，每个都要检索，结果需要合并去重
5. **性能优化**：需要在改写质量和延迟之间平衡，合并分类+改写减少 LLM 调用
...
async def rewrite_query(query, chat_history):
    # Step 1: 基于规则的复杂度分级（不调用 LLM）
    complexity = classify_complexity(query)
    if complexity == QueryComplexity.TRIVIAL:
        return [query]  # 简单问候，跳过改写
    ...
    # Step 3: 合并分类+改写（单次 LLM 调用，优化后的实现）
    if complexity in (QueryComplexity.MEDIUM, QueryComplexity.COMPLEX):
        query_type, rewritten = await _classify_and_rewrite(working_query)
    else:
...
> 先理解 `classify_complexity()` 的规则分级逻辑，再看 `_classify_and_rewrite()` 的合并 LLM 调用，最后看 `SimilarityValidator` 的校验机制。这是整个检索系统中最"智能"的部分。
...
Wiki Agent 需要将每一步执行轨迹提交给评估引擎，用于多维度质量评分。需要在业务代码中合理地集成数据采集逻辑。

### 为什么难

1. **采集点分散**：检索、记忆、决策、执行等不同节点都需要记录不同类型的轨迹
2. **自动采集 vs 手动记录**：SDK 可以自动采集 LLM 调用和节点执行，但检索细节、记忆写入等需要手动记录
3. **生命周期管理**：评估会话的 start/finish 需要在正确的时间点调用
4. **降级容错**：SDK 不可用时不能影响业务流程

### 现有解法

在 `graph.py` 中直接调用 `sdk.collector` 方法，无需额外中间层：

```python
# graph.py — 直接使用 SDK TrajectoryCollector
...
```python
# SDK 不可用时 get_collector() 返回 None，所有调用自动跳过
collector = get_collector()
if collector and collector.enabled:
    collector.record_retrieval(...)  # SDK 可用时才记录
```

### 学习建议

> 理解 SDK TrajectoryCollector 的 record/start/finish 生命周期，再看 graph.py 如何在 search/respond/decide/execute 各节点中调用 collector 方法。
...
| 10 | 难点 9: 评估数据采集 | SDK collector、生命周期管理 | 30 min |
...
第八阶段: graph.py + SDK TrajectoryCollector
           评估集成：在 graph.py 中直接调用 collector 接入评估平台
```
...
5. preload_embedding_model()  → 预加载 Embedding 模型
6. preload_reranker_if_enabled() → 预加载重排模型
7. start_env_monitor()        → 启动文件变化监控
```
...
|--------|------|
| 读什么 | SQLite 表结构 |
| 重点 | `sessions` 表（id, name, key_facts）、`messages` 表（role, content, wiki_results, extraction）、`user_memory` 表（id, facts） |
| 关键认知 | 会话和消息用 SQLite 存储，key_facts 是会话级的长期记忆，user_memory 是跨会话的用户级记忆 |
...
| 重点 | `merge_session_key_facts()`（去重 + 淘汰，最多 20 条）、`merge_user_memory()`（去重 + 淘汰，最多 30 条）、`get_session()`（带缓存） |
...
| 重点 | `hybrid_search()`（并行召回 + RRF 融合 + 重排）、`_rrf_merge()`（RRF 融合公式）、`semantic_search()`（Milvus 搜索 + BM25 降级） |
...
| 关注点 | 说明 |
|--------|------|
| 读什么 | 两阶段优化 Pipeline（规则分级 + LLM 合并分类改写） |
| 重点 | `rewrite_query()`（主入口）、`classify_complexity()`（规则分级）、`_classify_and_rewrite()`（合并 LLM 调用）、`Contextualizer`（指代消解）、`SimilarityValidator`（相似度校验） |
| 关键认知 | 原始 query 往往不适合直接检索，需要改写优化 |

**读完你应该知道**：

```
rewrite_query(query, chat_history) 的流程：

1. classify_complexity() — 基于规则的复杂度分级（不调用 LLM）
   TRIVIAL → 简单问候，跳过 RAG
   SIMPLE  → 简单查询，单次搜索，不改写
   MEDIUM  → 中等查询，1-2 次改写
   COMPLEX → 复杂查询，完整 Pipeline

2. Contextualizer — 指代消解（检测到代词才触发）
   "它怎么优化？" → "Milvus 向量数据库怎么优化？"

3. _classify_and_rewrite() — 合并分类+改写（单次 LLM 调用）
   直接输出 QueryType + 改写查询列表
   direct:    [q]
...
|--------|------|
| 读什么 | 四路记忆的合并逻辑和预算裁剪 |
| 重点 | `retrieve_context()`（四路检索入口）、`build_context_block()`（预算裁剪组装） |
| 关键认知 | LLM 的上下文由四部分组成：用户记忆 + 会话记忆 + RAG 检索结果 + 对话历史 |
...
总预算 ~3000 字符
├─ 优先级 1: 用户记忆（600 字符，固定保留）
├─ 优先级 2: 会话记忆（400 字符，固定保留）
├─ 优先级 3: wiki_results（1200 字符，动态分配）
└─ 优先级 4: history_summary（800 字符，用剩余预算）
```
...
### 8.1 `app/wiki_agent/agent/graph.py` + SDK TrajectoryCollector ⭐
...
| 4 | 四路上下文预算裁剪 | 3000 字符预算内按优先级分配 | `context_retriever.py` |
...
| 9 | 评估数据采集集成 | 在 graph.py 中直接调用 SDK TrajectoryCollector | `graph.py` |
...
| `context_retriever.py` | ~152 | **四路记忆合并** | ⭐⭐⭐ |
...
═══════════════════════════════════════════════════════════════════════
  ...
  │  ② NODE_EXECUTE         │──→ 节点执行记录
  │  ③ STATE_CHANGE         │──→ 状态变化记录
  │  ④ record_evidence()    │──→ EVIDENCE ──→ 最终回复记录
  └────────┬────────────────┘
           │
           ▼
  ┌─────────────────────────┐
...
```
graph.py（业务代码 + 评估采集）        collector（SDK）
─────────────────────────────        ──────────────
     │                                    │
     │  collector.record_retrieval()      │  构建 step dict
     │  collector.record_memory_write()   │  Pydantic 校验
     │  collector.record_evidence()       │  追加到缓冲区
     │                                    │
     └────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────┤
│ 外部知识库（External KB）                       │
│ - 存储：Milvus + BM25 + Markdown 文件           │
│ - 生命周期：永久                                 │
│ - 内容：知识库文档的检索结果                     │
│ - 检索方式：语义搜索 + BM25 + RRF 融合 + 重排   │
├─────────────────────────────────────────────────┤
│ 工作记忆（Working Memory）                      │
│ - 存储：内存（最近 10 条消息）                   │
│ - 生命周期：当前对话                             │
