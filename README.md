...
│  ┌──────────────────────────────────────────────┐   │
│  │  Redis Cache Layer (可选)                      │   │
│  │  报表缓存 · LLM 结果缓存 · 接口限流 · 会话缓存  │   │
│  └──────────────────────────────────────────────┘   │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────┴──────────────────────────────┐
...
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────┴──────────────────────────────┐
│  SQLite / PostgreSQL · Redis · TrajectoryCollector   │
└─────────────────────────────────────────────────────┘
...
```
├── app/
│   ├── evaluators/       # 6 个评估器 + BaseEvaluator + Consensus + LLM 缓存
...
│   ├── api/              # auth_middleware + rate_limit_middleware + workspace
│   ├── services/         # EvaluationService 业务逻辑 + 缓存集成
│   ├── models/           # Pydantic schemas + ActionType 常量
│   ├── db/               # SQLAlchemy ORM + async session
│   ├── core/             # pydantic-settings 配置 + Redis 缓存层 (cache.py)
│   ├── wiki_agent/       # 集成 Wiki-Agent（RAG + 自动评估 + Redis 会话缓存）
│   └── main.py           # FastAPI 入口
...
**后端**: Python 3.11+ · FastAPI · LangGraph · LangChain · SQLAlchemy (async) · Pydantic · httpx · Redis (可选)
