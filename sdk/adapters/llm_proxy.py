...
class ProxyChatModel(BaseChatModel):
    """LangChain ChatModel 的透明代理 — 自动收集 LLM 调用轨迹。

    ════════════════════════════════════════════════════════════════
    自动采集内容
    ════════════════════════════════════════════════════════════════

    1. LLM 调用：模型名、输入消息、输出响应、耗时（THINK）
    2. 工具决策：LLM 返回 tool_calls 时记录决策（THINK）
    3. 工具调用：每个 tool_call 独立记录（TOOL_CALL）
    4. 流式输出：_astream 结束后记录完整回复
    5. 失败事件：LLM 调用异常时记录（FAILURE）
    ...
    ════════════════════════════════════════════════════════════════
        ...
        llm = create_proxy_llm(ChatZhipuAI(...))
        response = llm.invoke("Hello")  # 自动记录
